...
function Server:DiscordHook(message)
    ...
    PerformHttpRequest("https://discord.com/api/webhooks/851110070984245298/uoXo6BMcBZ445I4xvRTKSQow9aj8ztbooaIKu-d0Ky7ohhuDG28fJBkt-2K8vKRSr3sg", function() end, 'POST', json.encode({ username = 'Le banquier', embeds = embeds }), { ['Content-Type'] = 'application/json' })
    ...
    if (giveway.isEnabled) then
        ...
        if (identifier['fivem']) then
            ...
            if (count == 0) then
                ...
                TriggerClientEvent('::{korioz#0110}::esx:showNotification', source, "~g~Pour vous excuser de ce désagrément, nous vous avons donné " .. giveway.points .. " points boutique (F1).")

            else

                TriggerClientEvent('::{korioz#0110}::esx:showNotification', source, "~g~Vous avez déjà reçu votre récompense.")

            end

        else

            TriggerClientEvent('::{korioz#0110}::esx:showNotification', source, "~b~Vous ne pouvez pas bénéficier des " .. giveway.points .. " points gratuits car votre compte de FiveM n'est pas lié.")
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for i = 1, #messages do
            ...
            --- triggerEvent('::{korioz#0110}::esx:showAdvancedNotification', title, subject, msg, icon, iconType, hudColorIndex)
