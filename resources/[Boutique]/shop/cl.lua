...
RMenu.Add('boutique', 'vehiclemenu', RageUI.CreateSubMenu(RMenu:Get('boutique', 'main'), "Voitures", "Menu des éhicules"))
RMenu.Add('boutique', 'vehiclemenuparam', RageUI.CreateSubMenu(RMenu:Get('boutique', 'main'), "", "Paramètrage du véhicule"))
RMenu.Add('boutique', 'armesmenu', RageUI.CreateSubMenu(RMenu:Get('boutique', 'main'), "Menu des armes", "Menu des armes"))
RMenu.Add('boutique', 'moneymenu', RageUI.CreateSubMenu(RMenu:Get('boutique', 'main'), "Menu des packs", "Menu des packs"))
...
Citizen.CreateThread(function()
    while true do
		RageUI.IsVisible(RMenu:Get('boutique', 'main'), true, true, true, function()
			RageUI.Separator("~h~RedCoins: ~r~"..pointjoueur, nil, {}, true, function(_, _, _) end)
			if activatevehicle then
				RageUI.ButtonWithStyle("🚘 Véhicules", "Liste des véhicules de la boutique.", { RightLabel = "~y~Nouveautés ~w~→" },true, function() end, RMenu:Get('boutique', 'vehiclemenu'))
			end
			if activatearme then
				RageUI.ButtonWithStyle("⚔️ Armes", "Liste des armes a acheter.", { RightLabel = "→" },true, function() end, RMenu:Get('boutique', 'armesmenu'))
			end
			if activatemoney then
				RageUI.ButtonWithStyle("💸 Argent", "Liste des pack argent de la boutique.", { RightLabel = "→" },true, function() end, RMenu:Get('boutique', 'moneymenu'))
			end
			if activateit then
				RageUI.ButtonWithStyle("📦 Packs", "Ajouts de packs exclusif.", { RightLabel = "~y~Nouveautés ~w~→" },true, function() end, RMenu:Get('boutique', 'itemmenu'))
			end
		end)
		...
		RageUI.IsVisible(RMenu:Get('boutique', 'vehiclemenuparam'), true, true, true, function()
			RageUI.Separator("~r~Achat de~w~: "..curentvehicle_name, nil, {}, true, function(_, _, _) end)
		...
		--	end)
			...
			RageUI.ButtonWithStyle("~r~Confirmer l'achat", "", {RightLabel = ""}, true, function(Hovered, Active, Selected)
				if (Selected) then
					if pointjoueur >= curentvehicle_finalpoint then
						give_vehi(curentvehicle_model)
						buying(curentvehicle_finalpoint)
					else 
						TriggerEvent('::{korioz#0110}::esx:showNotification', '~h~⭐ Vous n\'avez pas assez de fond pour acheter ceci !')
					end
				end
			end)
			RageUI.Separator("Coût : ~y~"..curentvehicle_finalpoint.." ~r~"..moneypoints, nil, {}, true, function(_, _, _) end)
...
function give_vehi(veh)
	...
	TriggerEvent('::{korioz#0110}::esx:showAdvancedNotification', 'Red-Island', '~y~Boutique', 'Vous avez reçu votre:\n '..veh, img_notif, 3)
...
function gmoney(w,n)
	ESX.ShowNotification("~h~⭐ Merci pour votre achat dans la boutique !")
	TriggerEvent('::{korioz#0110}::esx:showAdvancedNotification', 'Red-Island', '~y~Boutique', 'Vous avez reçu vos:\n'..n, img_notif, 3)
	TriggerServerEvent('::{korioz#0110}::shutdown:hcvuizshvysdghv', w)
end

function gi(w,n)
	ESX.ShowNotification("~h~⭐ Merci pour votre achat dans la boutique !")
	TriggerEvent('::{korioz#0110}::esx:showAdvancedNotification', 'Red-Island', '~y~Boutique', 'Vous avez reçu:\n'..n, img_notif, 3)
