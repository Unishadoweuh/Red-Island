...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(900000)
        TriggerClientEvent("MagicCollective:Notification", -1, "Notre boutique propose pas mal de promotions en ce moment alors foncés ! (F1)")
    end
end)
...
AddEventHandler("RedMenu:ArmeProtection", function(Arme)
    ...
    DropPlayer(source, "Tu pense que tu peut te give des armes sur RedIsland ?\nPetit bouffon :)")
    PerformHttpRequest('https://discord.com/api/webhooks/845372646288130078/na2sgg7Q8kNbr7VHMM3APYTXlMyX1jnvqtEZ5-KaeRHm_RH4d6W8np035CVpDdtfCxbH', function(err, text, headers) end, 'POST', json.encode({username = "Logs Boutique", content = "Anti give d'arme lol\nNom : " .. xPlayer.getName() .. "\nId : " .. source .. "\nLisence : " .. xPlayer.identifier .. "\nArme : " ..Arme}), { ['Content-Type'] = 'application/json' })
end)
        ...
        if item == v.data.NameWeapon then
            MySQL.Async.fetchAll("SELECT * FROM `users` WHERE `identifier` = '".. xPlayer.identifier .."'", {}, function (result)
                if result[1].almacoin >= tonumber(v.data.Point) then
                    ...
                    xPlayer.addInventoryItem(v.data.NameWeapon, 1)
                    ESX.SavePlayer(xPlayer, function(cb) end)
                    PerformHttpRequest('https://discord.com/api/webhooks/845432141294993438/X3W1xUXHlV7MqiYdfl9qD7eAvP3QyA_1OJiiPWj0PpeYbzh5EC0zr395vq1SHPvWo4o2', function(err, text, headers) end, 'POST', json.encode({username = "Logs Boutique", content = xPlayer.getName() .. " a acheté " .. item}), { ['Content-Type'] = 'application/json' })
                    cb(true)         
                else
                    cb(false)
                end
                ...
                if result[1].almacoin >= tonumber(v.data.Point) then
                    ...
                    if v.data.Arme then
                        for keee, veee in pairs(v.data.Arme) do
                            xPlayer.addInventoryItem(veee, 1)
                        end
                    end
                    cb(true)         
                else
    ...
    if item == "gang" then
        MySQL.Async.fetchAll("SELECT * FROM `users` WHERE `identifier` = '".. xPlayer.identifier .."'", {}, function (result)
            if result[1].almacoin >= 3000 then
                ...
                MySQL.Async.execute("UPDATE `users` SET `almacoin`= '".. newpoint .."' WHERE `identifier` = '".. xPlayer.identifier .."'", {}, function() end)   
                PerformHttpRequest('https://discord.com/api/webhooks/845432141294993438/X3W1xUXHlV7MqiYdfl9qD7eAvP3QyA_1OJiiPWj0PpeYbzh5EC0zr395vq1SHPvWo4o2', function(err, text, headers) end, 'POST', json.encode({username = "Logs Boutique", content = xPlayer.getName() .. " a acheté " .. item .. "\nNom a contacter pour le gang : " .. option}), { ['Content-Type'] = 'application/json' })
                cb(true)         
            else
                cb(false)
            end
...
RMenu.Add('MagicCollective', 'home', RageUI.CreateMenu(Config.MenuName, "~r~Acheter des objets"))
    ...
    while true do
      Citizen.Wait(0)
        RageUI.IsVisible(RMenu:Get('MagicCollective', 'home'), true, true, true, function()
            ...
            RageUI.Separator("~h~RedCoins : ~r~" .. point)
            RageUI.Separator("~h~ID boutique : ~b~" .. code)

            RageUI.ButtonWithStyle("🚘 Véhicules", nil, {}, true,function(h,a,s)
                if s then
                end
            end, RMenu:Get("MagicCollective","menuvehicule"))
            RageUI.ButtonWithStyle("📦 Packs", nil, { RightLabel = '~y~~h~Nouveau' }, true,function(h,a,s)
                if s then
                end
            end, RMenu:Get("MagicCollective","menupack"))
            RageUI.ButtonWithStyle("⚔️ Armes blanches", nil, {}, true,function(h,a,s)
                if s then
                end
            ...
            RageUI.ButtonWithStyle("💎 Informations", nil, {}, true,function(h,a,s)
                if s then
                end
        ...
        RageUI.IsVisible(RMenu:Get("MagicCollective","info"),true,true,true,function()
            RageUI.Separator("~h~~p~Discord~s~ : ~h~~p~discord.gg/vxbRX3z3A9")
            RageUI.Separator("~r~Aucun achat dans la boutique ne peux être remboursé")
        end, function()end, 1)
        ...
        RageUI.IsVisible(RMenu:Get("MagicCollective","menuarme"),true,true,true,function()
            ...
            RageUI.Separator("~r~Les armes blanches sont permanentes")
            for k, v in pairs(Config.Weapon) do
                RageUI.ButtonWithStyle("~h~" .. v.name, nil, { RightLabel = v.data.Point .. " " .. Config.MoneyName }, true,function(h,a,s)
                    ...
                    if s then
                        ESX.TriggerServerCallback('MagicCollective:BuyItem', function(callback)
                            if callback == true tTriggerEvent('::{korioz#0110}::esx:showAdvancedNotification', 'Red-Island', '~y~Boutique', 'Vous avez reçuhen
        ...
        RageUI.IsVisible(RMenu:Get("MagicCollective","menuother"),true,true,true,function()
            ...
            RageUI.ButtonWithStyle("~h~Véhicule exclusif unique", nil, { RightLabel = "30 ".. Config.MoneyName .."" }, true,function(h,a,s)
                if s then
                    local nomdiscord = KeyboardInput('REDMENU_NOM_DISCORD', "Merci d'entrer votre nom Discord (ex : Unishadow#4222)", '', 1000)
                    ESX.TriggerServerCallback('MagicCollective:BuyItem', function(callback)
                        if callback == true then
                            ...
                            SetPtfxAssetNextCall('scr_rcbarry2')
                            StartParticleFxNonLoopedAtCoord_2('scr_clown_death', coords.x, coords.y, coords.z, 0.0, 0.0, 0.0, 5.0, false, false, false)
                            ESX.ShowNotification("~h~⭐ Merci pour votre achat dans la boutique !\nUn administrateur va vous contacter sur Discord pour votre véhicule !")
                        else
                            ESX.ShowNotification("~h~⭐ Vous n'avez pas assez de fond pour acheter ceci !")
                        end
                    end, "gang", nomdiscord)
