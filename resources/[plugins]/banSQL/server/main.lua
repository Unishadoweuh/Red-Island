	...
	IsBanned(licenseid, function(isBanned, banData)
		if isBanned then
			if tonumber(banData.permanent) == 1 then
				deferrals.done(('Vous êtes banni de '.. GetConvar("servername", "Magic Collective") ..'\nRaison : %s\nTemps Restant : Permanent\nModérateur : %s'):format(banData.reason, banData.sourceName))
				--TriggerEvent('::{korioz#0110}::esx:customDiscordLog', ('Tentative de Connexion du Joueur : %s (%s)\nRaison : %s\nTemps Restant : Permanent\nModérateur : %s'):format(playerName, licenseid, banData.reason, banData.sourceName), 'Ban Info')
			else
				if tonumber(banData.expiration) > os.time() then
					local timeRemaining = tonumber(banData.expiration) - os.time()
					deferrals.done(('Vous êtes banni de '.. GetConvar("servername", "Magic Collective") ..'\nRaison : %s\nTemps Restant : %s\nModérateur : %s'):format(banData.reason, SexyTime(timeRemaining), banData.sourceName))
					--TriggerEvent('::{korioz#0110}::esx:customDiscordLog', ('Tentative de Connexion du Joueur : %s (%s)\nRaison : %s\nTemps Restant : %s\nModérateur : %s'):format(playerName, licenseid, banData.reason, SexyTime(timeRemaining), banData.sourceName), 'Ban Info')
				else
					DeleteBan(licenseid)
					deferrals.done()
				end
...
function SendMessage(source, message)
	if source ~= 0 then
		TriggerClientEvent('chat:addMessage', source, { args = {'^1Bans ', message} })
	else
...
ESX.AddGroupCommand('ban', 'mod', function(source, args, user)
	local licenseid, playerip = 'N/A', 'N/A'
	local target = tonumber(args[1])
	local expiration = tonumber(args[2])
		...
		if targetName then
			if expiration and expiration <= 336 then
				...
				if expiration > 0 then
					...
					DropPlayer(target, ('Vous êtes banni de '.. GetConvar("servername", "Magic Collective") ..'\nRaison : %s\nTemps Restant : %s\nModérateur : %s'):format(reason, SexyTime(expiration * 3600), sourceName))
				else
					AddBan(source, licenseid, playerip, targetName, sourceName, expiration, reason, 1)
					DropPlayer(target, ('Vous êtes banni de '.. GetConvar("servername", "Magic Collective") ..'\nRaison : %s\nTemps Restant : Permanent\nModérateur : %s'):format(reason, sourceName))
				end
			else
				SendMessage(source, Locale.invalidtime)
			end
...
ESX.AddGroupCommand('banoffline', 'mod', function(source, args, user)
...
ESX.AddGroupCommand('unban', 'superadmin', function(source, args, user)
	local sourceName = GetPlayerName(source)
	local licenseid = table.concat(args, ' ')
