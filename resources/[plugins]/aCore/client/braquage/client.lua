-- BOUFFON TA PAS LE COT├ë SERVEUR TU CROIS VRAIMENT VOLER LES SCRIPTS DE ADASTRA?
                ...
                if GetDistanceBetweenCoords(pos,resellerPos.vector,false) <= 150.0 then
                    ...
                    if GetDistanceBetweenCoords(pos,resellerPos.vector,false) <= 3.0 then
                        ...
                        if IsControlJustPressed(0, 51) then
                            ...
                            robbItems = {}
                            ESX.ShowAdvancedNotification("Bravo","~r~Cambriolage","Félicitations tu as reçus ~g~"..finalReward.."$ ~s~grace aux objets que tu as volé! Je te recontacterai","CHAR_DEVIN",9)
...
function terminate()
    ...
    c2 = "~b~"
    if #robbItems ~= 0 then
        ESX.ShowAdvancedNotification("Bravo","~r~Cambriolage","Bravo pour ton cambriolage, tu as ~b~"..#robbItems.." objets ~s~sur toi. Rends toi devant le reseller pour les revendres!","CHAR_DEVIN",9)
        ...
        robbItems = {}
        itemsSold = true
        ESX.ShowAdvancedNotification("Cambrioleur","~r~Cambriolage","T'es sérieux toi ? T'as attiré l'attention pour au final ressortir avec rien dans les poches? On se reverra...","CHAR_DEVIN",9)
    end
    exited = true
    Citizen.SetTimeout((1000*60)*robberiesConfiguration.houses[currentRobbingData.houseIndex].timeToGoToReseller, function()
        if not itemsSold then
            if resellerNPC ~= nil then DeleteEntity(resellerNPC) end
            ESX.ShowAdvancedNotification("Cambrioleur","~r~Cambriolage","T'es vraiment une merde... Tu fais déplacer un reseller pour au final pas te pointer? J'en ai trop vu, à la prochaine!","CHAR_DEVIN",9)
