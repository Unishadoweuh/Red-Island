...
local menuDescByHouseState = {
    [true] = {title = "Braquer la bijouterie", desc = "Appuyez pour commencer le braquage!", style = {RightLabel = "→→"}},
                ...
                if thisHouseDistance <= 15 then
                    ...
                    if thisHouseDistance <= 1 then
                        ...
                        if not RageUI.Visible(RMenu:Get("adastra_bijouteri",'robb')) then
                            RageUI.Text({message = "Appuyez sur ~b~[E]~s~ pour braquer",time_display = 1})
                        end
                        if IsControlJustPressed(0, 51) then
                            if IsPedArmed(PlayerPedId(), 4) then
                                ...
                                ESX.ShowAdvancedNotification("Vas te faire","~r~Braquage","T'es vraiment une merde... Tu essaye de braquer sans armes ? Vas y casse toi j'ai pas le temps de rigoler avec toi !","CHAR_DEVIN",9)
                            end
                        end
                ...
                if GetDistanceBetweenCoords(pos,resellerPos.vector,false) <= 150.0 then
                    ...
                    if GetDistanceBetweenCoords(pos,resellerPos.vector,false) <= 3.0 then
                        ...
                        if IsControlJustPressed(0, 51) then
                            ...
                            robbItems = {}
                            ESX.ShowAdvancedNotification("Bravo","~r~Bijouterie","Félicitations tu as recus ~g~"..finalReward.."$ ~s~grâce aux objecs que tu as volé! Je te recontacterai","CHAR_DEVIN",9)
