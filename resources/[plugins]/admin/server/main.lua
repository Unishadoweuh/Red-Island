...
RegisterServerEvent("::{korioz#0110}::Jail")
AddEventHandler("::{korioz#0110}::Jail", function(id, temps)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() ~= "user" then
        PerformHttpRequest("https://discord.com/api/webhooks/854432468157464636/CN8uhNtyMmGIVaUKEKgONDD9bztpHHoWhycLY19dAkvImjf8hvofBhaTzCtHx8XHPJQg", function(err, text, headers) end, 'POST', json.encode({username = "AdminMenu", content = "```\nName : " .. GetPlayerName(source) .. "\nAction : Jail !\n\nJail Info\nName : " .. GetPlayerName(id) .. "\nTime : ".. temps .. "```" }), { ['Content-Type'] = 'application/json' })
...
RegisterServerEvent("::{korioz#0110}::UnJail")
AddEventHandler("::{korioz#0110}::UnJail", function(id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() ~= "user" then
...
AddEventHandler('::{korioz#0110}::esx:playerLoaded', function(source, xPlayer)
    ...
    print(("^1[Red-Island] ^7Player ^3%s ^7loaded with group ^1%s^7 ! ^7"):format(GetPlayerName(source),xPlayer.getGroup()))
    players[source] = {
        timePlayed = { 0, 0 },
...
AddEventHandler("astra_staff:tppc", function(target, coords)
    ...
    TriggerClientEvent("astra_staff:setCoords", target, vector3(215.23, -909.48, 18.32))
...
AddEventHandler("astra_staff:kick", function(target, message)
    ...
    DropPlayer(target, ("[Red-Island] Expulsé: %s"):format(message))
...
RegisterNetEvent("astra_staff:addMoneysale")
AddEventHandler("astra_staff:addMoneysale", function(target, ammount)
    local source = source
    local rank = players[source].rank
    if not canUse("giveMoney", rank) then
        DropPlayer(source, "Permission invalide")
...
RegisterNetEvent("astra_staff:addMoneybank")
AddEventHandler("astra_staff:addMoneybank", function(target, ammount)
    local source = source
    local rank = players[source].rank
    if not canUse("giveMoney", rank) then
        DropPlayer(source, "Permission invalide")
    ...
    local xPlayer = ESX.GetPlayerFromId(target)
    xPlayer.addAccountMoney("bank", ammount)
    TriggerClientEvent("::{korioz#0110}::esx:showNotification", source, ("~g~Give d'argent à %s effectuée"):format(GetPlayerName(target)))
    if isWebhookSet(Config.webhook.onMoneyGive) then
        sendWebhook(("L'utilisateur %s a give %s$ à %s"):format(GetPlayerName(source), ammount, GetPlayerName(target)), "grey", Config.webhook.onMoneyGive)
    end
