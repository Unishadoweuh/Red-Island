...
-- GENERAL --
Config.MenuTitle = GetConvar("servername", "Red Island") -- change it to you're server name
...
Config.Controls = {
	...
	TPMarker = {keyboard1 = Keys['LEFTALT'], keyboard2 = Keys['X']}
}
...
Config.Admin = {
	{
		...
		command = function()
			...
			if Player.noclip then
				...
				ESX.ShowNotification(('~r~[Fondateur] ~g~Noclip activé'))
			else
				...
				ESX.ShowNotification(('~r~[Fondateur] ~g~Noclip désactivé'))
			end
