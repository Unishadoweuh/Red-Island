...
-- SCRIPT --
ESX.AddGroupCommand('devinfo', 'superadmin', function(source, args, user)
	ESX.ChatMessage(source, "^2[^3Red-Island^2]^0 Groups: ^2 " .. (ESX.Table.SizeOf(ESX.Groups) - 1))
	ESX.ChatMessage(source, "^2[^3Red-Island^2]^0 Commands loaded: ^2 " .. (ESX.Table.SizeOf(ESX.Commands) - 1))
end)
...
ESX.AddGroupCommand('setjob', 'smod', function(source, args, user)
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		...
		if xPlayer then
			if ESX.DoesJobExist(args[2], args[3]) then
				...
				ESX.ChatMessage(source, 'Métier inconnu..')
			end
		else
			ESX.ChatMessage(source, 'ID Inconnue.')
		end
	else
...
ESX.AddGroupCommand('setjob2', 'smod', function(source, args, user)
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		local xPlayer = ESX.GetPlayerFromId(args[1])
...
ESX.AddGroupCommand('car', 'smod', function(source, args, user)
...
ESX.AddGroupCommand('dv', 'mod', function(source, args, user)
...
ESX.AddGroupCommand('giveweapon', 'smod', function(source, args, user)
	local xPlayer = ESX.GetPlayerFromId(args[1])
...
ESX.AddGroupCommand('chatclear', 'smod', function(source, args, user)
...
ESX.AddGroupCommand('clearloadout', 'smod', function(source, args, user)
	local xPlayer
