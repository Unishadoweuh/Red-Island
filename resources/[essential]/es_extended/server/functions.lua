...
function ESX.SavePlayer(xPlayer, cb)
	...
	if xPlayer then
		...
		Async.parallel(asyncTasks, function(results)
			print(('[^2SAUVEGARDE^7] %s^7'):format(xPlayer.getName()))
			xPlayer.showNotification("💻 ~s~~h~Synchronisation Effectuée", 25)
...
function ESX.ChatMessage(source, msg, author, color)
	TriggerClientEvent('chat:addMessage', source, {color = color or {158, 0, 0}, args = {author or 'Red-Island', msg or ''}})
end
