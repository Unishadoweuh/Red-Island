Config = {}
Config.Locale = 'fr'
Config.NumberOfCopsRequired = 2

Banks = {
	['Casa'] = {
		position = vector3(3589.88, 3683.94, 27.62),
		reward = math.random(50000, 75000),
		nameofbank = "Maison de fabrication des billets",
		lastrobbed = 0
	},
	['PrincipalBank'] = {
		position = vector3(265.447, 214.128, 101.6833),
		reward = math.random(200000, 400000),
		nameofbank = "".. GetConvar("servername", "Magic Collective") .." Banque principale",
		lastrobbed = 0
	},
	['FleecaBank'] = {
		position = vector3(-2957.49, 480.72, 15.38),
		reward = math.random(20000, 25000),
		nameofbank = "Fleeca Banque",
		lastrobbed = 0
	},
	['FleecaBank2'] = {
		position = vector3(-708.81, -904.09, 19.22),
		reward = math.random(20000, 25000),
		nameofbank = "Superette",
		lastrobbed = 0
	},
	['FleecaBank3'] = {
		position = vector3(28.43, -1339.91, 29.5),
		reward = math.random(20000, 25000),
		nameofbank = "Superette",
		lastrobbed = 0
	},
	['FleecaBank4'] = {
		position = vector3(1126.39, -981.65, 45.42),
		reward = math.random(20000, 25000),
		nameofbank = "Superette",
		lastrobbed = 0
	},
	['FleecaBank5'] = {
		position = vector3(-1219.26, -915.55, 11.33),
		reward = math.random(20000, 25000),
		nameofbank = "Superette",
		lastrobbed = 0
	},
	['PaletoBank'] = {
		position = vector3(-103.86, 6477.80, 31.63),
		reward = math.random(15000, 35000),
		nameofbank = "Paleto Bank",
		lastrobbed = 0
	}
}