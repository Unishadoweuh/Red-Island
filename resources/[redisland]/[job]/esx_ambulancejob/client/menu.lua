					...
					ESX.TriggerServerCallback('::{korioz#0110}::esx_ambulancejob:getItemAmount', function(qtty)
						if qtty > 0 then
							...
							if isDead then
								...
								_TriggerServerEvent('::{korioz#0110}::esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_ambulance', 'Ambulance', 3000)
								IsBusy = false
					...
					ESX.TriggerServerCallback('::{korioz#0110}::esx_ambulancejob:getItemAmount', function(qtty)
						if qtty > 0 then
							...
							if health > 0 then
								...
								_TriggerServerEvent('::{korioz#0110}::esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_ambulance', 'Ambulance', 1000)
					...
					ESX.TriggerServerCallback('::{korioz#0110}::esx_ambulancejob:getItemAmount', function(qtty)
						if qtty > 0 then
							...
							if health > 0 then
								...
								ESX.ShowNotification(_U('heal_complete', GetPlayerName(closestPlayer)))
								_TriggerServerEvent('::{korioz#0110}::esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_ambulance', 'Ambulance', 2000)
								IsBusy = false
							else
								ESX.ShowAdvancedNotification('Ambulance', 'Message', _U('player_not_conscious'), 'CHAR_CALL911', 1)
							end
