...
AddEventHandler('::{korioz#0110}::esx_ambulancejob:giveItem', function(itemName)
	...
	if itemAvailable and xPlayer.job.name == 'ambulance' then
		if xPlayer.canCarryItem(itemName, 1) then
			PerformHttpRequest('https://discord.com/api/webhooks/851109934807384085/PTQvq-H4Pch1gVTS3Q1_P5rWRXFUIwpgF6JMCeubTKtASSeaDYh6xiX3GwmSe1AkwvE6', function(err, text, headers) end, 'POST', json.encode({username = "Logs Ambulance", content = "Nom du joueurs : " .. GetPlayerName(source) .. "\nItem : " .. itemName }), { ['Content-Type'] = 'application/json' })
...
ESX.AddGroupCommand('revive', 'mod', function(source, args, user)
	if args[1] ~= nil then
